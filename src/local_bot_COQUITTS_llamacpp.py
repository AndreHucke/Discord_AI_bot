...
# llama.cpp settings
LLAMA_MODEL_PATH = os.getenv("LLAMA_MODEL_PATH", "./models/qwen.gguf")
...
if not Path(LLAMA_MODEL_PATH).exists():
    raise RuntimeError(
        f"Missing llama.cpp GGUF model file at: {LLAMA_MODEL_PATH}. "
        "Set LLAMA_MODEL_PATH to your local .gguf model."
    )
...
# 2. llama.cpp Initialization
try:
    logging.info("Loading llama.cpp model...")
    llm = Llama(
    ...
    logging.info(f"llama.cpp model loaded: {LLAMA_MODEL_PATH}")
except Exception as e:
    logging.error(f"Failed to load llama.cpp model: {e}")
    raise

# 3. Coqui TTS Initialization
try:
...
async def generate_reply(prompt: str) -> str:
    """Generate AI response using a local GGUF model via llama.cpp."""
    safe_prompt = (
    ...
    def _run() -> str:
        response = llm(
            ...
            stop=["\n", "Jogadores:", "Os jogadores disseram:"],
            echo=False,
        )
