...
# Initialized for CUDA to leverage GPU acceleration with float16 precision
stt_model = WhisperModel(WHISPER_MODEL_SIZE, device="cuda", compute_type="float16")

# Audio Capture Settings
CHUNK_SIZE = 1024
FORMAT = pyaudio.paInt32  # 32-bit int
...
# 2 & 3. In-Memory Processing + VAD Filter
async def transcribe_audio_memory(audio_data: np.ndarray) -> str:
    ...
    # Extract text from the generator
...
async def handle_stop(ctx):
    ...
    try:
        ...
        # Convert to float32 and normalize to range between -1.0 and 1.0
        ...
        # Resample from device rate to 16000 Hz
